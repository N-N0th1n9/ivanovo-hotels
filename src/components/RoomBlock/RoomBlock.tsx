...
const RoomBlock: FC<RoomProps> = ({
	...
	return (
		<div className={`py-5 ${styles.mainBlock}`}>
			<div
				className={`w-full flex gap-16 py-16 pl-16 pr-12 rounded-2xl 3xl:pl-14 3xl:pr-11
          max-w-[1900px]:pr-4 max-w-[1900px]:pl-2 ${styles.subBlock}`}
			...
			>
				...
				<div
					className={`text-2xl basis-[60%] flex flex-col justify-between max-h-[541px] ${styles.textBlock}`}
				>
					<div style={{ color: bg }}>
						...
						<ul className='font-normal mb-10'>
							{room.features.map(feature => (
								<li className='flex gap-1 mb-1' key={feature}>
									<span>—</span>
									{feature}
								</li>
							))}
					...
					<button
						className={`px-[75px] text-black py-4 font-bold rounded-xl w-[80%] max-w-[416px] bg-white border-white
